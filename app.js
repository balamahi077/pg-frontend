...
// Handle Login Submission (Secure Backend Validation & Loading State)
...
if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        ...
        // 1. Save original button text and set Loading State
        const originalBtnText = submitBtn.innerHTML;
        submitBtn.disabled = true;
        submitBtn.classList.add('opacity-75', 'cursor-not-allowed');
        submitBtn.innerHTML = `
            <svg class="animate-spin h-5 w-5 inline mr-2 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        try {
            ...
            // Remove loading state on server error
            submitBtn.disabled = false;
            submitBtn.classList.remove('opacity-75', 'cursor-not-allowed');
            submitBtn.innerHTML = originalBtnText;
